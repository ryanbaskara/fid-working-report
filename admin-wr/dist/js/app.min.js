...
// custom function
  ...
  $(function () {
	...
	function openEditAttended(id_edit){

	  var row = document.getElementById('rows'+id_edit);

	  var cells = row.getElementsByTagName("td");

	  $('#edit_title').html(cells[2].innerText+", "+cells[1].innerText);

	  $('#edit_place').val(cells[8].innerText);

	  $('#edit_activity').val(cells[9].innerText);

	  $('#edit_date').val(cells[10].innerText);

	  if(cells[3].innerText == "" || cells[4].innerText == "" || cells[5].innerText == ""){

	    $('#edit_time_in').val("08:00:00");

	    $('#edit_time_out').val("17:00:00");

	    $('#edit_time_break').val("01:00:00");

	  }else{

	    $('#edit_time_in').val(cells[3].innerText);

	    $('#edit_time_out').val(cells[4].innerText);

	    $('#edit_time_break').val(cells[5].innerText);
	...
	function openEditWR(line){

	  $('#edit_title').html(json[line].name);

	  $('#edit_employee_id').val(json[line].employee_id);

	  $('#edit_date').val(json[line].date);

	  $('#edit_customer_name').val(json[line].customer_name);

	  $('#edit_wo_number').val(json[line].wo_number);

	  $('#edit_project_name').val(json[line].project_name);
	...
	function openEditEmployee(line){
		var status = json[line].status;
		if (status == 1) {
	  ...
	  $('#edit_login_id').val(json[line].login_id);

	  $('#edit_login_id_hidden').val(json[line].login_id);

	  $('#edit_password').val(json[line].password);

	  $('#edit_name').val(json[line].name);

	  $('#edit_employee_id').val(json[line].id);

	  $('#edit_position').val(json[line].position);

	  $('#edit_join_date').val(json[line].join_date);

	  $('#edit_end_date').val(json[line].end_date);

	  $('#edit_start_period').val(json[line].start_peroid_po);

	  $('#edit_end_period').val(json[line].end_period_po);

	  $('#edit_pm').val(json[line].pm_leader);

	  $('#edit_level').val(json[line].level);

	  $('#edit_role').val(json[line].role);

	  $('#edit_skill').val(json[line].skill);

	  $('#edit_wbs').val(json[line].wbs);

	  $('#edit_remark').val(json[line].remark);

	  $('#edit_company').val(json[line].company_name);
